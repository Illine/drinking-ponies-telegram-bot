...
class InsightServiceImpl(
    ...
    override fun build(
        ...
        logger.debug("Building insight for telegram user [{}]", telegramUserId)

        // Single fetch covers both streak (up to STREAK_LIMIT_DAYS) and insight window (INSIGHT_WINDOW_DAYS).
        ...
        val text = renderInsight(byDate, today, dailyGoalMl, streak)
    ...
    private fun renderInsight(
        ...
        val windowStart = today.minusDays((INSIGHT_WINDOW_DAYS - 1).toLong())
        val windowTotals = (0 until INSIGHT_WINDOW_DAYS).map { offset ->
            val date = windowStart.plusDays(offset.toLong())
            date to (byDate[date] ?: 0)
        }
        val avgMl = windowTotals.sumOf { it.second } / INSIGHT_WINDOW_DAYS
        val bestDay = windowTotals
            .filter { it.second > 0 }
    ...
    private companion object {
        ...
        const val INSIGHT_WINDOW_DAYS = 7
    }
