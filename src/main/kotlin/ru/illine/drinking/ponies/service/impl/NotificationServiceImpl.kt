...
class NotificationServiceImpl(
    ...
    override fun start(messageContext: MessageContext) {
        ...
        SendMessage(
            messageContext.chatId().toString(),
            TelegramMessageConstants.START_DEFAULT_SETTINGS_MESSAGE.format(setting.delayNotification.displayName)
        ).apply { sender.execute(this) }
    }
        ...
        val sendMessageFunction: () -> Unit = {
            SendMessage(
                ...
                replyMarkup = TelegramBotKeyboardHelper.timeOptionButtons(
                    PauseTimeNotificationType.entries.filter {
                        it == PauseTimeNotificationType.RESET || it.minutes > delayNotification.minutes
                    }
                )
            }.apply { sender.execute(this) }
        }
    ...
    override fun suspendNotifications(notifications: Collection<NotificationSettingDto>) {
        ...
        notifications
            .forEach {
                SendMessage(
                    it.telegramChat.externalChatId.toString(),
                    TelegramMessageConstants.NOTIFICATION_SUSPEND_MESSAGE.format(it.delayNotification.displayName)
                ).apply {
