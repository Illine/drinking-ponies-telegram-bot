...
interface NotificationRepository : JpaRepository<NotificationEntity, Long> {
    ...
    @Query(
        value = "select count(n.id) > 0 from notifications n where n.user_id = :userId",
        nativeQuery = true
    )
    fun existsByUserId(@Param("userId") userId: Long): Boolean

    @Query(
        value = "select n.deleted from notifications n where n.user_id = :userId",
        nativeQuery = true
    )
    ...
    @Modifying
    @Query(
        value = "update notifications set deleted = :deleted where user_id = :userId",
        nativeQuery = true
    )
