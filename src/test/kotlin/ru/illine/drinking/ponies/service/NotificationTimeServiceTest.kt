...
class NotificationTimeServiceTest @Autowired constructor(
    ...
    fun `isNotificationDue table test`(
        ...
        delay: DelayTimeNotificationType,
        expected: Boolean,
        description: String
    ) {
        getMutableClock().setTime(currentTimeIso)
    ...
    companion object {
        ...
        fun notificationDueScenarios(): Stream<Arguments> {
            return Stream.of(
                ...
                Arguments.of(
                    "2025-01-01T12:00:00Z", "2025-01-01T10:00:00Z", DelayTimeNotificationType.TWO_HOURS, true,
                    "Exact boundary: 10:00 + 120m == 12:00, boundary is inclusive [<=] -> Due"
                ),

                Arguments.of(
                    "2025-01-01T13:00:00Z", "2025-01-01T10:00:00Z", DelayTimeNotificationType.HOUR, true,
                    "Exact boundary: 12:00 + 60m == 13:00, boundary is inclusive [<=]  -> Due"
                ),

                Arguments.of(
                    "2025-01-01T12:00:01Z", "2025-01-01T10:00:00Z", DelayTimeNotificationType.TWO_HOURS, true,
                    "1 second after due time -> Due"
                ),

                Arguments.of(
                    "2025-01-01T11:59:59Z", "2025-01-01T10:00:00Z", DelayTimeNotificationType.TWO_HOURS, false,
                    "1 second before due time -> Not Due"
                ),

                Arguments.of(
                    "2025-01-01T15:00:00Z", "2025-01-01T10:00:00Z", DelayTimeNotificationType.HOUR, true,
                    "Long overdue: 10:00 + 60m < 15:00 -> Due"
                ),

                Arguments.of(
                    "2025-01-02T01:00:00Z", "2025-01-01T23:00:00Z", DelayTimeNotificationType.HOUR, true,
                    "Day rollover: 23:00 + 60m = 00:00 (next day) < 01:00 -> Due"
                ),

                Arguments.of(
                    "2025-01-01T12:00:00Z", "2025-01-01T11:50:00Z", DelayTimeNotificationType.HALF_HOUR, false,
                    "Short delay not reached: 11:50 + 30m = 12:20 > 12:00 -> Not Due"
                ),

                Arguments.of(
                    "2025-01-01T10:00:00Z", "2025-01-01T12:00:00Z", DelayTimeNotificationType.HOUR, false,
